...
async function processSegmentTypeAsync(
  ...
  const extractSegmentsFromSource = async (sourceData: RawJsonData, sourceName: string) => {
    // Special handling for "By Region" and "By State" segment types - use geography names as parent hierarchy
    // Exclude "By Country" to show countries as flat list without "Europe" parent
    const isRegionSegmentType = segmentType === 'By Region' || segmentType === 'By State'
  ...
  for (let i = 0; i < allPaths.length; i += batchSize) {
    ...
    for (const { path: pathArray, data } of batch) {
      ...
      // SPECIAL HANDLING for "By Region" and "By State" segment types:
      ...
      // Exclude "By Country" to keep countries as segments, not geographies
      const isRegionSegmentType = segmentType === 'By Region' || segmentType === 'By State'
...
export async function processJsonDataAsync(
  ...
  try {
    ...
    const metadata: Metadata = {
      market_name: 'Containerized Second Life ESS',
